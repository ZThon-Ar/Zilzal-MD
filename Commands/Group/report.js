    ...
    start: async (
      ...
      { text, prefix, isBotAdmin, isAdmin, pushName, metadata, args }
        ...
        if(!m.isGroup){
            ...
            try {
                userPfp = await Zed.profilePictureUrl(m.sender, "image");
              } catch (e) {
                userPfp = botImage3;
              }
            ...
            for (let i = 0; i < devs.length; i++) {
              await Zed.sendMessage(devs[i],{image: {url: userPfp}, caption: reportMessage,mentions: [m.sender],});
            }
        }
        else{
            if (!args[0]) return m.reply(`Please provide a message to report Developers !`);
            ...
            try {
              ...
              for (let i = 0; i < devs.length; i++) {
                await Zed.sendMessage(devs[i],{image: {url: ppgc}, caption: reportMessage,mentions: [m.sender],});
            }
        }
    }
}
