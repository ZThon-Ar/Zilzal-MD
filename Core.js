...
/ Powered By: Team Zilzal                                                            /
/-----------------------------------------------------------------------------------/
/             Meet Team Zilzal who holds all rights to this repository:              /
/                                                                                   /
...
module.exports = async (Zed, m, commands, chatUpdate, store) => {
  try {
    ...
    const metadata = isGroup ? await Zed.groupMetadata(from) : {};
    const pushname = m.pushName; //|| 'NO name'
    const participants = isGroup ? metadata.participants : [sender];
    const groupAdmin = isGroup
      ? participants.filter((v) => v.admin !== null).map((v) => v.id)
      : [];
    const botNumber = await Zed.decodeJid(Zed.user.id);
    ...
    global.suppL = "https://t.me/ZedThon";
    ...
    if (!isCreator) {
      ...
      if (
        ...
        return m.reply(
          `- لقـد تم حظـرك من استخـدام اوامـر البـوت لـ السبب *${checkban.reason}* من *${checkban.gcname}*`
        );
    }
      ...
      if (m.isGroup && mongoschema == "true") {
        ...
        if (budy.includes(`https://chat.whatsapp.com/${linkgce}`)) {
          ...
          Zed.sendMessage(
            from,
            {
              text: `\`\`\`「  نظـام منـع الروابـط  」\`\`\`\n\n@${
                kice.split("@")[0]
              } تم طـرده .. بنجـاح من هنـا بسبب ارسـاله روابـط مجموعـات هنـا .. وتم حـذف الرابـط المرسـل بنجـاح.`,
              mentions: [kice],
            },
            {
              quoted: m,
    ...
    if (botSwitchGC) {
      ...
      if (
        ...
        return console.log(
          `\nتم تعطيـل البـوت في المجموعـة n ${groupName} .. بنجـاح\n`
        );
      }
    }
    ...
    if (body.startsWith(prefix) && !icmd) {
      ...
      await Zed.sendMessage(m.from, reactmxv);

      Zed.sendMessage(m.from, {image: {url: botImage1,},caption: mikutext,}, {
        quoted: m,
      });
    }
    ...
    if (
      ...
      await Zed.sendMessage(m.from, reactionMess).then(() => {
        return Zed.sendMessage(m.from, buttonmess, {
          quoted: m,
        });
      });
    }
    if (cmd.react) {
      ...
      await Zed.sendMessage(m.from, reactm);
    }
